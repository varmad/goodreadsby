...
//   Phase 3 (Publish):   The branch is pushed to GitHub and a pull request is
//                        opened against PR_BASE_BRANCH. No agent involved — this
//                        is a plain command, so the PR always reflects the
//                        reviewed code.
//
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  try {
    ...
    // Phase 3: Publish
    //
    // Push the branch and open its pull request. This runs as a command rather
    // than an agent so the result is deterministic: one branch, one PR, every
    // iteration. gh-pr reads its config from the sandbox environment, which
    // Sandcastle populates from .sandcastle/.env: GH_TOKEN to authenticate,
    // GITHUB_REPO (or the origin remote) for the target, PR_BASE_BRANCH for
    // the base.
    // -----------------------------------------------------------------------
    const publish = await sandbox.exec("gh-pr", {
      onLine: (line) => console.log(line),
    });

    if (publish.exitCode !== 0) {
      // Stop rather than continue: a publish failure is nearly always a bad
      // token or a missing remote, so every later iteration would fail the same
      // way and pile up unpushed branches. The work is safe on `branch` — fix
      // the cause and re-run to publish it.
      console.error(publish.stderr.trim());
      throw new Error(
        `Publish failed (exit ${publish.exitCode}). Branch ${branch} is committed locally but has no PR.`,
      );
    }
  } finally {
    await sandbox.close();
